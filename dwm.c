...
void clientmessage(XEvent *e) {
  ...
  if (showsystray && cme->window == systray->win &&
    ...
    if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
      ...
      // TODO added these two lines to give systray a class name to control
      // through picom
...
0); if (my + HEIGHT(c) < m->wh) my += HEIGHT(c); } else { h = (m->wh - ty) / (n
- i); resize(c, m->wx + mw, m->wy + ty, m->ww - mw - (2*c->bw), h - (2*c->bw),
