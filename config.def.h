...
static const char *colors[][3] = {
    ...
    [SchemeStatus] = {col_gray4, col_gray1,
                      "#000000"}, // Statusbar right {text,background,not used
                                  // but cannot be empty}

    [SchemeTagsSel] = {col_info_blue, col_gray1,
...
static Key keys[] = {
    ...
    {MODKEY | ShiftMask, XK_v, spawn,
     SHCMD("mpv --profile=low-latency /dev/video0")},
...
/* bindsym $mod+F2		        exec --no-startup-id screenkey -s small
 * --scr 1 -p fixed -g 600x100+2573+1330 --opacity .9 --font-color white */
